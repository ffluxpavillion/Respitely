...
const MealsTimelineOld = () => {
  ...
  // ----------------------- NEW CODE: FETCHING DROP-INS FROM SERVER-----------------------
  useEffect(() => {
  ...
  useEffect(() => { // Fetch drop-ins for today
    const today = moment().format('dddd').toLowerCase();
    let mealsForToday = [];
    const dailyList = allDropinsToday;
    // console.log('MEALSFORTODAY=======', mealsForToday);
    ...
    // console.log('OLD SORTEDMEALS=====', sortedMeals);
...
export default MealsTimelineOld;
